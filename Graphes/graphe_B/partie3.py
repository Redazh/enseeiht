...
def matrice_adh(df,d):
    ...
    for i in range(num_nodes):
        for j in range(i + 1, num_nodes):
            distance_carre = ((df.iloc[i]['x'] - df.iloc[j]['x'])**2 +
                        (df.iloc[i]['y'] - df.iloc[j]['y'])**2 +
                        (df.iloc[i]['z'] - df.iloc[j]['z'])**2)
            if np.sqrt(distance_carre) <= d:
                cout = distance_carre
                M[i, j] = cout
                M[j, i] = cout
    return M
def calculer_carateristiques(df, d, afficher):
    ...
    # Degré moyen
    degree_noeud = np.sum((M>0), axis=1)
    ...
    if afficher.lower() == "o":
        ...
        plt.xlabel('Degré')
        plt.ylabel('Nombre de noeuds')
        plt.title('Distribution du clustering du graphe', fontsize=10)
        plt.show()
    ...
    for source in G.nodes():
        courts_chemins[source] = {}
        for target in G.nodes():
            if source != target and nx.has_path(G, source, target):
                shortest_path = nx.shortest_path(G, source, target, weight='weight')
                courts_chemins[source][target] = len(shortest_path) - 1
                
    print(f"Les chemin les plus court du graphe sont: {courts_chemins}")
    ...
    def nb_pluscourtschemins(graph, source, target):
        ...
        if nx.has_path(graph, source=source, target=target):
            ...
            sp[(source, target)] = []
            for path in nx.all_shortest_paths(graph, source=source, target=target,weight='weight'):
