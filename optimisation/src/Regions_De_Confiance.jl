...
function regions_de_confiance(f::Function, gradf::Function, hessf::Function, x0::Vector{<:Real};
    ...
    while !arret
        ...
        if algo_pas == "cauchy"
            ...
            sk =  gct(gradf(x_sol), hessf(x_sol), Δk,max_iter = max_iter_gct)
        end
        mk0 = f(x_sol)
        mkSk = f(x_sol) + (gradf(x_sol)')*sk + 0.5*sk'*hessf(x_sol)*sk
        rhok = (f(x_sol)-f(x_sol+sk))/(mk0-mkSk)
        ...
        if norm(gradf(x_sol)) <= max(tol_rel * norm(gradf(x0)) , tol_abs)
            flag = 0
            arret = true
        elseif x_solAchange && ((norm((x_sol - xk))) <= (epsilon * max((tol_rel * norm(xk)) , tol_abs)))
            flag = 1
            arret = true
        elseif x_solAchange && ((abs(f(x_sol)-f(xk))) <= (epsilon * max(tol_rel * abs(f(xk)) ,tol_abs)))
            flag = 2
            arret = true
