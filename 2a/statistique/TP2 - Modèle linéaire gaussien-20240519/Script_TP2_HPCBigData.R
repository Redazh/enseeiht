...
#L'erreur standard des résidus est de 16.78, ce qui donne une idée de la dispersion des résidus autour de la droite de régression.

x11()
plot(aix$O3p,aix$O3o,xlab="O3 PREVU MOCAGE",ylab="O3 OBSERVE",
     main="PREVISIONS 24H",pch='+')
abline(regsimple,col="red")
##Examinez à quel point les points sont dispersés autour de la droite de régression. 
#Une dispersion large indique que le modèle ne capture pas bien la variabilité des données.
x11()
plot(aix$O3o,type ="l",lwd=2,main="Concentration d'ozone � Aix",xlab="Date",ylab="[O3]")
points(aix$O3o,col="blue",pch="+")
...
x11()
pairs(data[,c(-1,-7)])
# -> on cherche � reperer sur cette matrice graphique d'une part les pb de colinearite entre predicteurs et d'autre part les fortes correlations entre 
...
summary(regmult)
# -> l'analyse des tests de Student montre que l'influence de NO2 n'est pas significative, malgre notre pre-traitement
# le % de variance expliquee passe � 51%, ce qui n'est pas formidable
model.matrix(regmult)[1:10,]


plot(fitted(regmult),residuals(regmult),main="Hypoth�se d'homosc�dasticit�",xlab="Valeurs ajust�es (Y*)",ylab="R�sidus")
abline(h=0, col="red")
...
# Si les points sont répartis de manière homogène autour de la ligne rouge, l'hypothèse d'homoscédasticité est vérifiée. 
# Une forme de cône ou d'éventail indique une hétéroscédasticité (variance non constante des erreurs).
...
# les queues de distribution s'eloignent de la normalite, mais globalement l'hypoth�se de normalite ne pose pas de probleme sur la plage classique des valeurs prises par O3o

acf(residuals(regmult))
# -> autocorrelation marquee pendant une bonne semaine, moins nette ensuite, hypoth�se d'independance des erreurs non respectee
#Les barres du graphique ACF devraient idéalement se situer dans les limites de confiance (environ 95%). Si des barres sortent de ces limites, cela indique une autocorrélation significative des résidus, ce qui viole l'hypothèse d'indépendance.

plot(fitted(regmult),data$O3o,xlab="[03] PREVUE par AS",ylab="[03] OBSERVEE",pch="+",main="Hypoth�se de lin�arit�")
...
# Le terme constant du modele englobe donc ici l'effet moyen de la modalite F. L'estimation du parametre alpha(S) relatif � la modalite S du facteur JJ
# R estime l'effet diff�rentiel de la modalit� S par rapport � la modalit� F (0.4716).
...
# On ajoute le facteur STATION a 5 modalites. On cherche donc ici � estimer un seul modele statistique capable d'adapter sa prevision selon au site considere.
...
# O3o se comporte differemment aux stations Pla et Cad par rapport � Aix alors que Als et Ram ont des comportements moyens tr�s proches d'Aix. 
...
# On remarque que la variable FF semble plus influente qu'� la question 2, de meme pour NO2. FF et NO2 doivent donc avoir un impact significatif sur O3o sur certains sites seulement.
...
summary(regaic)
formula(regaic)
# la s�lection automatique descendante utilisant le critere AIC ne retire que le facteur JJ, comme la selection manuelle precedente.
...
# la penalisation de la complexite du modele est plus severe avec l'indice BIC et m�ne en general � des modeles plus parcimonieux (moins de predicteurs selectionnes qu'avec le critere AIC)
# Le % de variance expliquee passe � 52%
...
legend(0,330,lty=1,col=c("black"),legend=c("observ�"),bty="n")
...
# Ces scores ont ete calcules sur fichier d'apprentissage, c'est � dire sur les donnees qui ont servi � l'estimation du modele statistique.
...
# Les scores sur fichier test servent � apprecier la robustesse du modele (= capacite � s'adapter � de nouvelles donnees).
...
# le modele sera trop adapte � l'echantillon d'apprentissage et incapable de s'en detacher (=sur-apprentissage, incpacite a generaliser). 
