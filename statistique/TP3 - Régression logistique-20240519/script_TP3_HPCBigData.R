...
# creation du predictand OCC et de la prevision brute de depassement de seuil OCCp
...
# On exclut O3o qui ne peut etre exploite comme un predicteur exploitable (O3o=mesures du lendemain, donnees indisponibles au moment du calcul de la prevision statistique en conditions operationnelles)
...
#scores(fitted(glm.outBIC)>0.5,data$OCC)
# PSS=0.35, H=0.37, F=0.02

# Exploitation de la courbe ROC pour choisir le seuil de proba a exploiter (on cherche a maximiser le score PSS)
...
# Le seuil de proba optimal a exploiter est ici tres different de 0.5 (il depend de l'archive d'apprentissage)
...
#Remarque :
# L'evaluation sur fichier test pourrait �galement etre pratiquee comme au TP2, le script CV.R pouvant etre adapte
